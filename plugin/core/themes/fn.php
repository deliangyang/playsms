...
function themes_hint($text) {
	...
	if (!$ret) {
		$ret = "<i class='glyphicon glyphicon-info-sign sms-tooltip' data-toggle=tooltip data-placement='right' title='" . $text . "' rel=tooltip></i>";
	}
...
function themes_mandatory($text) {
	...
	if (!$ret) {
		$ret = $text . " <i class='glyphicon glyphicon-exclamation-sign sms-mandatory' data-toggle=tooltip data-placement='right' title='" . _('This field is required') . "' rel=tooltip></i>";
	}
...
function themes_select($name, $options = array(), $selected = '', $tag_params = array(), $css_id = '', $css_class = '') {
	...
	$css_id = (trim($css_id) ? trim($css_id) : 'sms-select-' . core_sanitize_alphanumeric($name));
	...
	$ret = $js . PHP_EOL . '<select name="' . $name . '" id="' . $css_id . '" class="sms-select ' . $css_class . '" ' . $params . '>' . $select_options . '</select>';
...
function themes_dialog($content = array(), $title = '') {
	...
	foreach ($contents as $type => $data) {
		...
		foreach ($data as $text) {
			if (trim($text)) {
				$message .= '<div class=sms-dialog-text>' . trim($text) . '</div>';
				$continue = TRUE;
			}
		}
...
function themes_select_users_single($select_field_name, $selected_value = '', $tag_params = array(), $css_id = '', $css_class = '') {
	...
	if (!$ret) {
		...
		$css_id = (trim($css_id) ? trim($css_id) : 'sms-select-users-single-' . core_sanitize_alphanumeric($select_field_name));
		...
		$ret = $js . PHP_EOL . '<select name="' . $select_field_name . '" id="' . $css_id . '" class="sms-select ' . $css_class . '" ' . $params . '>' . $option_user . '</select>';
...
function themes_select_account_level_single($status = 2, $select_field_name, $selected_value = '', $tag_params = array(), $css_id = '', $css_class = '') {
	...
	if (!$ret) {
		...
		$css_id = (trim($css_id) ? trim($css_id) : 'sms-select-account-level-' . core_sanitize_alphanumeric($select_field_name));
...
function themes_input($type = 'text', $name = '', $value = '', $tag_params = array(), $css_id = '', $css_class = '') {
	...
	if (!$ret) {
		...
		$ret = '<input type="' . $type . '" name="' . $name . '" value="' . $value . '" id="' . $css_id . '" class="sms-input ' . $css_class . '" ' . $params . '>';
	}
