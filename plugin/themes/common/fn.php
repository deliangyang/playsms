...
function common_hook_themes_submenu($content = '') {
	...
	$ret = '<div>';
	$ret .= '<span class="sms-icon glyphicon glyphicon-user" alt="' . $tooltips_logged_in . '" title="' . $tooltips_logged_in . '"></span>' . $logged_in;
	$ret .= $separator . '<span class="sms-icon glyphicon glyphicon-credit-card" alt="' . $tooltips_credit . '" title="' . $tooltips_credit . '"></span><div id="submenu-credit-show">' . $credit . '</div>';
...
function common_hook_themes_buildmenu($menu_config) {
	...
	foreach ($menu_config as $menu_title => $array_menu) {
		...
		if (count($m)) {
			$main_menu .= "<li id='". $class ."' class='dropdown'><a href='#' data-toggle='dropdown' data-hover='dropdown' class='dropdown-toggle'>" . $menu_title . " <i class='tideenergy-dropdown fa fa-angle-down fa-2x' aria-hidden='true'></i></a>";
				...
				<div class='container'>
					...
					<div class='navbar-collapse collapse'>
						<ul class='nav navbar-nav'>
							<li><a href='" . _u(_HTTP_PATH_BASE_) . "'>" . _('Home') . "</a></li>
                            <li><a href='" . _u('index.php?app=main&inc=feature_phonebook&op=phonebook_list') . "'>" . _('Phonebook') . "</a></li>
							" . $main_menu . "
                            <li><a href='" . _u('index.php?app=main&inc=core_sender_id&op=sender_id_list') . "'>" . _('Sender ID') . "</a></li>
                            <li class='num-gs-spacing'><a href='" . _u(_HTTP_PATH_BASE_) . "'>" . _('Doc') . "</a></li>
                            <li><a href='" . _u(_HTTP_PATH_BASE_) . "'>" . _('Support') . "</a></li>
						</ul>
...
function common_hook_themes_navbar($num, $nav, $max_nav, $url, $page) {
	...
	if ($num) {
		...
		$nav_pages = "<div class=sms-nav-bar>";
