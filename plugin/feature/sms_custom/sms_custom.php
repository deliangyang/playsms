		...
		while ($db_row = dba_fetch_array($db_result)) {
			if ($owner = user_uid2username($db_row['uid'])) {
				...
				if ($db_row['sms_receiver']) {
					$sms_receiver = "<div name=sms_custom_sms_receiver><span class=\"sms-icon glyphicon glyphicon-inbox\" alt=\"" . _('Receiver number') . "\" title=\"" . _('Receiver number') . "\"></span>" . $db_row['sms_receiver'] . "</div>";
				}
				...
				$content .= "
					<tr>
						...
						<td>
							<div name=sms_custom_keywords><span class=\"sms-icon glyphicon glyphicon-search\" alt=\"" . _('Keywords') . "\" title=\"" . _('Keywords') . "\"></span>" . $db_row['custom_keyword'] . "</div>
							" . $sms_receiver . "
